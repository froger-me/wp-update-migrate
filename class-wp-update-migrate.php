	...
	class WP_Update_Migrate {
		...
		private function __construct( $package_handle, $package_prefix ) {
			...
			if ( ! wp_doing_ajax() ) {
				...
				$this->init_package_type( $package_handle );

				if ( 'plugin' === $this->package_type ) {
					require_once ABSPATH . 'wp-admin/includes/plugin.php';

					$update_hook        = 'plugins_loaded';
				...
				if ( ! is_null( $this->package_type ) ) {
					...
					if ( ! $is_version_current ) {
						...
						add_action( $update_hook, array( $this, 'update' ), -10, 0 );
					}
				}
			}
		...
		public static function get_instance( $package_handle, $package_prefix ) {
			...
			if ( ! $cache ) {
				...
				return new self( $package_handle, $package_prefix );
			}

			return $cache;
		}

		public function init_package_type( $package_handle ) {

			if ( false !== strpos( $package_handle, self::get_content_dir() . 'plugins' ) ) {
				$this->package_type = 'plugin';
			} else {
				$theme_obj = wp_get_theme( $package_handle );
		...
		public function update_failed_notice() {
			...
			$message .= '<p>' . sprintf( __( 'The %1$s will not have any effect until the issues are resolved.', 'wp-update-migrate' ), $this->package_type ) . '</p>';
